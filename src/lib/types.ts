...
export type HrAggregate = {
  ...
  sample_size: number;
  /**
   * Относительное изменение к прошлому окну, в процентах.
   *
   * null — сравнивать не с чем либо прошлый уровень был слишком мал,
   * чтобы процент что-то значил. Показывать «—» вместо «+100%» честнее:
   * второе выглядит как катастрофа, а означает, что раньше в отделе
   * почти никто ничего не писал.
   */
