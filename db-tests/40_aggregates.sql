...
begin
  ...
  -- Сравнивать не с чем — дельты нет. Не ноль: ноль означал бы
  -- «измерили и не изменилось», а здесь измерять было нечего.
  perform t_eq(v_finance.risk_delta_pct is null, true, 'без прошлого окна дельта пуста, а не нулевая');
end $$;
