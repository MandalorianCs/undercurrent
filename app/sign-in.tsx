    ...
    <Screen>
      ...
      {mode === 'corporate' && current === 'code' ? (
        <Card>
          ...
          <P muted>Отправили на {email}. Приходит за минуту, иногда попадает в спам.</P>
          {/* Два способа намеренно. На встроенной почте Supabase шаблон
              письма поменять нельзя, и приходит ссылка. Со своим SMTP
              в письмо кладётся код. Пока работают оба, экран не должен
              обещать чего-то одного — иначе человек ищет в письме то,
              чего там нет, и решает, что письмо пришло неправильное. */}
          <P muted>
            Если в письме ссылка — перейдите по ней, вы вернётесь сюда уже подтверждённым. Если код —
            введите его ниже.
          </P>
          <Field
            label="Код из письма"
